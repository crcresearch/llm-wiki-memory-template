...
name: wiki-source
description: >-
  Ingest a new external source document (paper, article, design doc, README,
  external reference) into the wiki as a source-summary page. Use when a new
  external document enters the project and should become durable memory. NOT
  for filing experiment results; use the wiki-experiment skill for those.
---
...
This is the `llm-wiki.md`-style Ingest. It is **not** for filing our own experiment results. For experiment results, use the `wiki-experiment` skill instead.
...
8. Append a `## [YYYY-MM-DD] ingest | Source title` entry to `log_{{REPO_NAME}}.md`. The first bullet is the attribution line `- by: <name> via cursor`, where `<name>` is the output of `git config user.name` in the wiki repo (read it, do not invent it). Then 2 to 5 bullets describing the ingest. See "Log Entry Attribution" in `SCHEMA_{{REPO_NAME}}.md`.
