...
name: wiki-lint
description: >-
  Run a health check on the wiki. Surfaces orphan pages, dead links, stale
  claims, missing frontmatter, untyped pages, and concepts that lack their own
  page. Reports findings and offers fixes. Use when the user asks to lint the
  wiki, notices broken links or contradictions, or after a large ingest.
---
...
6. For accepted fixes, apply them with cross-reference repair in both directions, update `index_{{REPO_NAME}}.md` as needed, and append a `## [YYYY-MM-DD] lint | Subject` entry to `log_{{REPO_NAME}}.md` describing what was cleaned up. The first bullet of that entry is the attribution line `- by: <name> via cursor`, where `<name>` is the output of `git config user.name` in the wiki repo (read it, do not invent it). See "Log Entry Attribution" in `SCHEMA_{{REPO_NAME}}.md`.
