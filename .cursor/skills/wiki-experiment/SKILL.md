...
description: >-
  File experiment results to the wiki following the SCHEMA Ingest procedure.
  Use after an experiment run produces metrics, ablations, or comparisons worth
  keeping as durable project memory.
---
...
- Which experiment / variant ran
- Configuration used (parameters, seeds, dataset, scale)
- Headline metrics (per hop count where applicable)
...
1. Read existing wiki pages for this experiment first (benchmark page, headline results page, comparison pages, concept pages whose claims the result bears on). Integrate rather than duplicate.
...
4. **Honest reporting.** Bad results, contradicted claims, and worsened metrics get filed truthfully, not polished. Never report accuracy from projections, only from real script outputs. The canonical "Universal Rationalizations (Always Wrong)" table lives at `wiki/agents/discipline-gates.md`.
...
8. Append a `## [YYYY-MM-DD] update | Experiment name` entry to `log_{{REPO_NAME}}.md`. The first bullet is the attribution line `- by: <name> via cursor`, where `<name>` is the output of `git config user.name` in the wiki repo (read it, do not invent it). Then 2 to 5 bullets describing the run. See "Log Entry Attribution" in `SCHEMA_{{REPO_NAME}}.md`.
...
Tell the user which wiki pages were created or updated, summarize the headline result in one sentence, and remind that the wiki commit is local.
