...
# .cursor/hooks/session-start.sh, with llm-wiki-memory-template substituted at
# install time.
#
...
This project uses the wiki at wiki/llm-wiki-memory-template.wiki/ as durable memory.
It is a separate git repository with its own remote, NOT a subdirectory of
the main repo. Read SCHEMA_llm-wiki-memory-template.md before non-trivial wiki edits.
...
Every wiki edit ends with a commit in the wiki's own repo:
  git -C wiki/llm-wiki-memory-template.wiki add <files>
  git -C wiki/llm-wiki-memory-template.wiki commit -m "..."
...
EOF
    ...
    INDEX_FILE="wiki/llm-wiki-memory-template.wiki/index_llm-wiki-memory-template.md"
    ...
    LOG_FILE="wiki/llm-wiki-memory-template.wiki/log_llm-wiki-memory-template.md"
    if [[ -f "$LOG_FILE" ]]; then
